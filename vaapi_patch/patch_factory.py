#!/usr/bin/env python3
"""Anchored patch: wire the VAAPI decoder factory into the wrapper's
rtc_peerconnection_factory_impl.cc so every PeerConnectionFactory built on
Linux gets VAAPI-first + FFmpeg-fallback decoding.
...
def patch(path: str) -> bool:
    ...
    # 1) Add the include (Linux only, since gstreamer is Linux-only).
    ...
    if "#include \"vaapi_video_decoder.h\"" in src:
        ...
        print("  [ok] added vaapi_video_decoder.h include")

    # 2) Swap the decoder factory in the #else branch (non-MSDK builds).
    factory_old = (
        "#else\n"
    ...
    factory_new = (
        ...
        "        webrtc::CreateBuiltinVideoEncoderFactory(),\n"
        "        CreateVaapiVideoDecoderFactory(\n"
        "            webrtc::CreateBuiltinVideoDecoderFactory()),\n"
        "#else\n"
        "        webrtc::CreateBuiltinVideoEncoderFactory(),\n"
        "        webrtc::CreateBuiltinVideoDecoderFactory(),\n"
    ...
    if "CreateVaapiVideoDecoderFactory(" in src:
        ...
        print("  [ok] wired CreateVaapiVideoDecoderFactory into #else branch")
