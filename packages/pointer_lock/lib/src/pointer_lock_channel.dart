...
class ChannelPointerLock extends PointerLockPlatform {
  ...
  Stream<PointerLockMoveEvent> _createRawStream({
    ...
    if (defaultTargetPlatform == TargetPlatform.windows) {
      ...
      // On Linux, the right stream depends on the display server:
      // - Wayland: gdk_pointer_grab is unavailable, so the pointer must be
      //   locked natively via zwp_pointer_constraints + relative pointer and
      //   deltas driven through the event channel (like macOS/Windows).
      // - X11: the Dart-controlled stream (gdk grab + warp) works fine.
      // Resolve lazily on listen so we can query the platform.
      return _createLinuxRawStream(unlockOnPointerUp: unlockOnPointerUp);
    } else {
      // On X11 (and other platforms), we are fine with Dart-controlled streams.
      return _createRawStreamDart(unlockOnPointerUp: unlockOnPointerUp);
    }
  }

  /// On Linux, picks the native Wayland event-channel stream or the
  /// Dart-controlled X11 stream once the display server is known.
  Stream<PointerLockMoveEvent> _createLinuxRawStream({
    required bool unlockOnPointerUp,
  }) {
    late final StreamController<PointerLockMoveEvent> controller;
    StreamSubscription<PointerLockMoveEvent>? subscription;
    controller = StreamController<PointerLockMoveEvent>(
      onListen: () async {
        final isWayland = await _isWayland();
        final raw = isWayland
            ? _createRawStreamNative(unlockOnPointerUp: unlockOnPointerUp)
            : _createRawStreamDart(unlockOnPointerUp: unlockOnPointerUp);
  ...
  /// Whether the Linux app runs on a Wayland display server.
  Future<bool> _isWayland() async {
    final result = await methodChannel.invokeMethod<bool>('isWayland');
    return result ?? false;
  }
  ...
  Stream<PointerLockMoveEvent> _createRawStreamNative({
    ...
    Offset convertEventToOffset(dynamic event) {
      // Native implementations deliver [dx, dy] either as a typed Float64List
      // (macOS/Windows) or as a plain list of doubles (Linux wayland-client
      // protocol headers via flutter_linux's standard codec). Accept both so
      // deltas are never silently dropped.
      if (event == null) {
        return Offset.zero;
      }
      if (event is Float64List) {
        if (event.length < 2) return Offset.zero;
        return Offset(event[0], event[1]);
      }
      if (event is List<num>) {
        if (event.length < 2) return Offset.zero;
        return Offset(event[0].toDouble(), event[1].toDouble());
      }
      return Offset.zero;
    }
