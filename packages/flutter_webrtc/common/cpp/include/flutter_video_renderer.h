...
class FlutterVideoRendererManager {
 public:
  ...
  // Out-of-line so the gl_renderers_ map (scoped_refptr<FlutterVideoRendererGL>)
  // is only destroyed where FlutterVideoRendererGL is complete (the .cc), not
  // in every TU that includes this header with just a forward declaration.
...
#if defined(__linux__)
  // GPU-resident renderers (OPENNOW_RENDERER=gl). Kept separate because they
