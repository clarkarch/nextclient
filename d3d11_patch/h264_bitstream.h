...
// for a raw GStreamer pipeline. Windows counterpart of vaapi_patch's
// vaapi_h264_bitstream.h (identical content, dependency-free).
//
...
//
// NOTE: the converter class is named D3d11AvccToAnnexB (not AvccToAnnexB) so
// this header can coexist with vaapi_patch's vaapi_h264_bitstream.h in a
// checkout where BOTH patches are layered onto the same libwebrtc wrapper —
// two definitions of the same class in one build would be an ODR hazard.
#ifndef LIBWEBRTC_SRC_D3D11_H264_BITSTREAM_H_
#define LIBWEBRTC_SRC_D3D11_H264_BITSTREAM_H_
...
#endif  // LIBWEBRTC_SRC_D3D11_H264_BITSTREAM_H_
